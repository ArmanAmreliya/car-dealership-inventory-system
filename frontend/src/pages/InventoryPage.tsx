...
import { RefreshCw, AlertCircle, PackageCheck, AlertTriangle, ShieldCheck } from 'lucide-react';
...
export function InventoryPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between pb-2">
        <div>
          ...
          <p className="mt-1 text-xs sm:text-sm font-medium text-slate-500 dark:text-slate-400">
            Real-time stock quantities, reorder points, and availability status.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl border border-slate-200/80 bg-white px-3.5 py-2.5 text-xs font-bold text-slate-700 shadow-subtle hover:bg-slate-50 disabled:opacity-50 transition-all dark:border-slate-800 dark:bg-slate-800 dark:text-slate-200"
          >
      ...
      {/* Error Notice */}
      {isError && (
        <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5 text-rose-800 flex items-start justify-between gap-4 shadow-subtle dark:border-rose-900/40 dark:bg-rose-950/40">
          <div className="flex items-center gap-3">
            <AlertCircle className="h-5 w-5 text-rose-600 shrink-0" />
            <div>
              <p className="text-sm font-bold">Failed to load inventory dataset</p>
              <p className="text-xs text-rose-700 dark:text-rose-300 mt-0.5">
                {(error as any)?.message || 'Could not communicate with the backend service.'}
              </p>
            </div>
          </div>
          <button
